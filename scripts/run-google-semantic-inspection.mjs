...
try {
  const result = await executeInspection(process.argv.slice(2), {
    ...
    directory:()=>{
      ...
      if(!s.isDirectory()||s.isSymbolicLink()||s.uid!==process.getuid()||
        (s.mode&0o777)!==0o700||realpathSync(d)!==d)throw Error('Unsafe directory');
      const names=readdirSync(d);
      if(names.length!==1||names[0]!=='google-address-inspection-001.held')throw Error('Existing evidence requires review');
      const p=join(d,names[0]);const f=lstatSync(p);
      if(!f.isFile()||f.isSymbolicLink()||f.uid!==process.getuid()||(f.mode&0o777)!==0o600)throw Error('Unsafe prior hold');
      const h=JSON.parse(readFileSync(p,'utf8'));
      if(h.packetId!=='google-address-inspection-001'||h.project!=='signmons'||
        h.requests!==1||h.liabilityMicros!==100000)throw Error('Prior hold mismatch');
      return d;
    },
    collect:collectAddress,
    confirm:prompt=>privateDialog('SEMANTIC TEST: Results will be shown privately, not saved. '+prompt),
    run:async(options,request)=>{
      ...
      const {reviewGoogleAddressResponse}=require('../dist/communications/google-address.adapter.js');
      const {reviewGoogleServiceArea}=require('../dist/communications/google-service-area.js');
      ...
      let summary;
      const runner=new GoogleAddressOneShot({...options,
        approval:{...options.approval,packetId:'google-semantic-inspection-002'},
        transport:{validate:async(input,signal)=>{
          const r=await transport.validate(input,signal);
          if(r.status==='RESPONSE'&&!signal.aborted)summary=await semanticReview(input,r.body,{
            address:reviewGoogleAddressResponse,county:reviewGoogleServiceArea,
          });
          return r;
        }},
      });
      const result=await runner.run(request);
      if(result.status==='OBSERVED'&&summary) {
  ...
  console.error('Semantic inspection refused or uncertain. Do not retry or reset holds.');
  process.exitCode=1;
}
