...
class AdminPipeTests(unittest.TestCase):
    def run_forward(self, mode="success", payload=b"FICTIONAL_ADMIN_CANARY\n", backup=False, real_node=False, migration=False, runtime=False):
        ...
        child = (
            ...
            + ("print('REFUSED',flush=True);sys.exit(1)" if mode == "early" else
               ...
               ("print(value.decode());sys.stderr.write(value.decode());sys.exit(1)"
                if mode == "failure" else "print(" + repr("RUNTIME_HANDOFF_READY" if runtime else "MIGRATION_COMPLETE" if migration else "BACKUP_COMPLETE" if backup else "HANDOFF_READY") + ",flush=True)"))
        )
        ...
        program = (
            ...
            "import json;forward_admin(json.loads(sys.argv[2]),0,1,0.3,backup=" + repr(backup) + ",migration=" + repr(migration) + ",runtime=" + repr(runtime) + ")"
        )
    ...
    def test_runtime_protocol_private_success_and_failure(self):
        code,output=self.run_forward(runtime=True)
        self.assertEqual(code,0)
        self.assertIn(b"Login remains disabled",output)
        for mode,payload in [("failure",b"FICTIONAL_ADMIN_CANARY\n"),("early",None),
                             ("success",None),("success",b"\x03"),("success","terminate")]:
            code,_=self.run_forward(mode,payload,runtime=True)
            self.assertNotEqual(code,0)
