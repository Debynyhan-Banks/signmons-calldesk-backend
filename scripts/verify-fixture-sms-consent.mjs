...
try {
  for (const width of [390, 1280]) {
    ...
    const model = new FixtureSmsConsent(credentials, () => policy);
    ...
    await page.locator("#smsRequested").check();
    ...
    await page.waitForFunction(() =>
      document
        .querySelector("#smsStatus")
        .textContent.includes("temporary fixture memory"),
    );
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        mode: "REAL_FIXTURE_MODEL_AND_TRANSPORT_MOCK_INTAKE",
        persistentDatabaseWrites: 0,
        providerCalls: 0,
        checks,
      },
      null,
