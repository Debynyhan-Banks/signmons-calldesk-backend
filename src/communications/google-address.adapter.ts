...
export class GoogleAddressAdapter {
  ...
  async preview(input: unknown) {
    ...
    try {
      const parsed = reviewGoogleAddressResponse(
        intended,
        await this.fixture(request),
      );
      return result(parsed.status, parsed.candidate);
    } catch {
...
/** In-process semantics only: no transport, retention, proof or fixture provenance.
 * Caller must not persist the transient correction candidate as provider content.
 */
export function reviewGoogleAddressResponse(input: unknown, raw: unknown) {
  const result = (
    status: "INVALID_INPUT" | "UNKNOWN" | "REVIEW" | "CORRECTION_REQUIRED",
    candidate: AddressCorrectionCandidate | null = null,
  ) => ({
    status,
    candidate,
  ...
  const intended = record(input);
  if (
    Object.keys(intended).sort().join(",") !== "city,postalCode,street,unit" ||
    !text(intended.street, 150) ||
    !text(intended.city, 60) ||
    typeof intended.unit !== "string" ||
    (intended.unit !== "" && !text(intended.unit, 30)) ||
    typeof intended.postalCode !== "string" ||
    !/^\d{5}(-\d{4})?$/.test(intended.postalCode) ||
    intended.street.length +
      intended.city.length +
      intended.unit.length +
      intended.postalCode.length +
      4 >
      280
  )
    return result("INVALID_INPUT");
  try {
    ...
    const correction =
      ...
      normalize([intended.street, intended.unit].filter(Boolean).join(" ")) !==
