...
import { CalendarAppliedRecoveryService } from "./calendar-applied-recovery.service";

describe("Inactive reviewed APPLIED CREATE recovery", () => {
  ...
  let service: CalendarAppliedRecoveryService;
  beforeEach(() => {
    jest.resetAllMocks();
    ...
    service = new CalendarAppliedRecoveryService(
  ...
  it.each(["owner", " ADMIN "])(
    ...
    async (role) => {
      ...
      const version = new Date(now.getTime() + 1);
  ...
  it.each([
    { action: "RESCHEDULE" },
    { action: "CANCEL" },
    ...["PENDING", "UNCERTAIN", "NEEDS_REVIEW", "FINALIZED", "ABORTED"].map(
      (status) => ({ status }),
    ),
    { finishedAt: now },
  ...
  it("remains absent from module wiring", () => {
    ...
    ).not.toContain("CalendarAppliedRecoveryService");
  });
});
