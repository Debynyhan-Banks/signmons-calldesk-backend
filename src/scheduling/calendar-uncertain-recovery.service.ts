...
/** Inactive, explicitly reviewed elapsed-grace UNCERTAIN CREATE read-back only.
 ...
 */
@Injectable()
export class CalendarUncertainRecoveryService {
  constructor(
    ...
    try {
      admittedAt = await this.prisma.$transaction(async (tx) => {
        ...
        if (
          ...
          throw this.changed();
        const notBefore = calendarReadbackNotBefore(operation);
        if (!calendarReadbackReady(notBefore)) throw this.changed();
        ...
        const claimed = await tx.calendarOperation.updateMany({
          ...
          data: { updatedAt: version, readbackNotBefore: notBefore },
        });
        ...
        await tx.auditLog.create({
          data: {
            ...
            metadata: {
              ...
              readbackNotBefore: notBefore.toISOString(),
            },
          },
        });
        return version;
  ...
  private changed() {
    return new ConflictException(
      "Calendar work changed, is not an UNCERTAIN CREATE, or its read-back wait has not elapsed. Refresh review state.",
    );
  }
  private uncertain() {
