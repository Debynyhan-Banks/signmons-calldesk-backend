...
## Required checks
From this backend checkout, run `SIGNMONS_GOVERNANCE_REPO=<resolved governance> node scripts/check-governance-baseline.mjs`. Missing governance or missing safeguards must fail, not skip. This invokes the frozen-baseline and cross-repository queue checks.
